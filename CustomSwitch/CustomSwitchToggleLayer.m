...
@implementation CustomSwitchToggleLayer
...
-(void) drawInContext:(CGContextRef)context
{
    ...
    // strings
	_font = _font ?: [UIFont boldSystemFontOfSize:ceilf(self.bounds.size.height * .6)];
    CGFloat textSpaceWidth = (self.bounds.size.width / 2) - (knobRadius / 2);
    
    UIGraphicsPushContext(context);
    
    // 'ON' state label (self.onString)
    CGSize onTextSize = [self.onString sizeWithFont:_font];
    ...
    CGSize offTextSize = [self.offString sizeWithFont:_font];
